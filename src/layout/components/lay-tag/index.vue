...
async function moveToView(index: number): Promise<void> {
  await nextTick()
...
function smoothScroll(offset: number): void {
  // 每帧滚动的距离
    ...
    >
      <div ref="tabDom" class="tab select-none" :style="getTabStyle">
        <div
          ...
          :ref="`dynamic${index}`"
          :key="index"
          class="scroll-item is-closable" :class="[
            linkIsActive(item),
