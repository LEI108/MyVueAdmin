...
export function usePublicHooks() {
  ...
  /** 处理状态颜色（待指派-橙; 处理中-蓝; 处理完毕-绿） */
  const processStatusTagStyle = computed(() => {
    return (status: number) => {
      const colors = {
        // 待指派-橙
        1: {
        ...
        2: {
          light: {
            '--el-tag-text-color': '#096dd9',
            '--el-tag-bg-color': '#e6f7ff',
            '--el-tag-border-color': '#91d5ff',
          },
          dark: {
            '--el-tag-text-color': '#409EFF',
            '--el-tag-bg-color': '#17242d',
            '--el-tag-border-color': '#274a57',
          },
        },
        // 处理完毕-绿
        3: {
      ...
      return isDark.value ? colors[status].dark : colors[status].light
    }
  })
